...
class MercadoPagoService {
    ...
    public function handlePayment(Request $request) {
        ...
        if ($payment->status === "approved") {
            ...
            return redirect()
                ->route('home')
                ->withSuccess(['payment' => "Gracias, {$name}. Recibimos su pago de {$originalAmount} {$originalCurrency} ({$amount} {$currency})."]);
        }

        return redirect()
            ->route('home')
            ->withErrors('No pudimos confirmar su pago. Por favor, vuelva a intentar.');
    }
